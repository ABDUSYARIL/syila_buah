...
@section('title', 'Ganti Password - Syila Buah')

@section('content')
<div class="max-w-md mx-auto px-6 py-8" x-data="{ show: { old: false, new: false, confirm: false } }">
    <div class="mb-8">
        ...
        <h1 class="text-3xl font-extrabold text-gray-dark tracking-tight mt-3">Ganti Password</h1>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-soft border border-gray-light p-8 hover:shadow-soft-hover transition-all duration-300">
        <form action="{{ route('profile.change-password.update') }}" method="POST" class="space-y-4">
            @csrf
            ...
            @foreach([
                ...
                <div class="flex flex-col gap-1.5">
                    ...
                    <div class="relative">
                        <div class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-muted">
                        ...
                        <div class="absolute right-3 top-1/2 -translate-y-1/2">
                            <button type="button" @click="show.{{ $field['key'] }} = !show.{{ $field['key'] }}" class="text-gray-muted hover:text-primary transition-colors cursor-pointer">
                                <span class="material-symbols-rounded text-lg" x-text="show.{{ $field['key'] }} ? 'visibility_off' : 'visibility'">visibility</span>
                            </button>
                        </div>
                    </div>
                </div>
            ...
            <!-- Action buttons -->
            <div class="flex gap-3 mt-8 border-t border-bg-light pt-6">
                <a href="{{ route('profile') }}" class="w-1/2 inline-flex items-center justify-center gap-2 font-bold rounded-xl border-2 border-primary text-primary hover:bg-green-light px-4 py-3 text-sm transition-all duration-300">
                    Batal
                </a>
                <button
                    type="submit"
